...
def _fix_tokenization(tokenized_sent, bert_embs, old_det_to_new_ind, obj_to_type, token_indexers, pad_ind=-1):
    ...
    # use for parse sentence to scene-graph
    new_tokenization = [token[0] for token in new_tokenization_with_tags]
    new_token_string = str(new_tokenization[0])
    for i in range(len(new_tokenization) - 1):
        new_token_string = new_token_string + ' ' + str(new_tokenization[i + 1])

    return text_field, tags, new_token_string
...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        if self.mode == 'rationale' and self.split == 'test':
            self.prase_question_path = os.path.join(VCR_ANNOTS_DIR, f'span_lead{self.conditioned_answer_choice}_{self.split}_q.h5')
            print("Load", self.prase_question_path)
        else:
            self.prase_question_path = os.path.join(VCR_ANNOTS_DIR, f'span_{self.mode}_{self.split}_q.h5')
        self.prase_answers_path = os.path.join(VCR_ANNOTS_DIR, f'span_{self.mode}_{self.split}_a.h5')
    ...
    def __getitem__(self, index):
        ...
        if 'endingonly' not in self.embs_to_load:
            questions_tokenized, question_tags, question_string = zip(*[_fix_tokenization(
                item['question'],
            ...
            #instance_dict['question_len'] = ArrayField(np.array([len(item['question'])],dtype=int))

        answers_tokenized, answer_tags, answer_string = zip(*[_fix_tokenization(
            answer,
        ...
        answers_len = [len(answer) for answer in answer_choices]
        #instance_dict['answers_len'] = ArrayField(np.array(answers_len, dtype=int))
        ...
        ################################################
        # corenlp  prase the text to sentence scene graph

        annot_id = str(item['annot_id'].split("-")[-1]) 
        with h5py.File(self.prase_question_path,'r') as f:
            question_node_idx = f[annot_id]['node_idx'][:]
            question_relate_idx = f[annot_id]['relate_idx'][:]
            question_relate_os = f[annot_id]['relate_so'][:]
        
        assert len(question_string[0].split()) == question_node_idx.shape[1]
        instance_dict['question_node_idx'] = ArrayField(question_node_idx, padding_value=0)
        instance_dict['question_relate_idx'] = ArrayField(question_relate_idx, padding_value=0)
        instance_dict['question_relate_os'] = ArrayField(question_relate_os, padding_value=-1)

        answers_node_idx = []
        answers_relate_idx = []
        answers_relate_os = []
        with h5py.File(self.prase_answers_path,'r') as f:
            for i in range(4):
                ans_node_idx = f[annot_id+'_'+str(i)]['node_idx'][:]
                ans_relate_idx = f[annot_id+'_'+str(i)]['relate_idx'][:]
                ans_relate_os = f[annot_id+'_'+str(i)]['relate_so'][:]
            
                assert len(answer_string[i].split()) == ans_node_idx.shape[1]

                ans_node_idx = ArrayField(ans_node_idx, padding_value=0)
                ans_relate_idx = ArrayField(ans_relate_idx, padding_value=0)
                ans_relate_os = ArrayField(ans_relate_os, padding_value=-1)
                answers_node_idx.append(ans_node_idx)
                answers_relate_idx.append(ans_relate_idx)
                answers_relate_os.append(ans_relate_os)

        instance_dict['answers_node_idx'] = ListField(answers_node_idx)
        instance_dict['answers_relate_idx'] = ListField(answers_relate_idx)
        instance_dict['answers_relate_os'] = ListField(answers_relate_os)
...
def collate_fn(data, to_gpu=False):
    ...
    td['question_node_mask'] = torch.any(td['question_node_idx'] > 0, -1).long()
    td['answers_node_mask'] = torch.any(td['answers_node_idx'] > 0, -1).long()
    td['question_relate_mask'] = torch.any(td['question_relate_os'] >= 0, -1).long()
    td['answers_relate_mask'] = torch.any(td['answers_relate_os'] >= 0, -1).long()
